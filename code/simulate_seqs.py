...
def generate_quasispecies(s, counts, p):
    # generates  quasispecies variants from sequences
    # with mutation probability p per base
    # specified by counts
    # being the original being the first count
    population = [s]
    for c in counts:
        mut = mutate(s,p)
        for i in range(c):
...
def readize(seqs,L=200,p=0.005,C=5):
...
def readize_random(seqs,L=200,p=0.005,N=1000):
    # given s, generate reads of length L
    # with error rate p
    # and coverage C
    ret = []
